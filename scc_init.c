...
int scc_init(file init, climchange_struct* scc)
{
	...
	********************************************************************/
	...
	if (!errflag && scan_value(init, &scc->s_tmax, 'd'))
	{
		printf("ERROR reading scalar for tmax, scc_init()\n");
		errflag=21401;
	}
	if (!errflag && scan_value(init, &scc->s_tmin, 'd'))
	{
		printf("ERROR reading scalar for tmin, scc_init()\n");
		errflag=21401;
	}
	if (!errflag && scan_value(init, &scc->s_prcp, 'd'))
	{
		printf("ERROR reading scalar for prcp, scc_init()\n");
		errflag=21401;
	}
	if (!errflag && scan_value(init, &scc->s_vpd, 'd'))
	{
		printf("ERROR reading scalar for vpd, scc_init()\n");
		errflag=21401;
	}
	if (!errflag && scan_value(init, &scc->s_swavgfd, 'd'))
	{
