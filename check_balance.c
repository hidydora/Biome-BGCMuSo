...
int check_water_balance(wstate_struct* ws, int first_balance)
{
	...
	/* sum of sources */
	ws->in = ws->prcp_src + ws->groundwater_src + ws->IRGsrc;
	
	/* sum of sinks */
	ws->out = ws->soilevap_snk + ws->snowsubl_snk + 
	...
	ws->store = ws->soilw_SUM + ws->pond_water + ws->snoww + ws->canopyw;
	
	/* calculate current balance */
	balance = ws->in - ws->out - ws->store;
...
int check_carbon_balance(cstate_struct* cs, int first_balance)
{
	...
	cs->in = cs->psnsun_src + cs->psnshade_src + cs->PLTsrc + cs->GRZsrc + cs->FRZsrc;
	
	/* sum of sinks: respiration, fire and management */
	cs->out = cs->leaf_mr_snk      + cs->leaf_gr_snk      + cs->froot_mr_snk     + cs->froot_gr_snk + 
	...
	/* sum of current storage */
	cs->store = cs->leafc      + cs->leafc_storage      + cs->leafc_transfer +
...
int check_nitrogen_balance(nstate_struct* ns, int first_balance)
{
	...
	ns->in = ns->nfix_src + ns->ndep_src + ns->SPINUPsrc +
			 ns->PLTsrc +  ns->GRZsrc +  ns->FRZsrc;
	
	
	/* sum of sinks: volatilization, fire, leach, diffusion, management */
	ns->out = ns->nvol_snk + ns->fire_snk + ns->nleached_snk + ns->ndiffused_snk + 
			  ns->GRZsnk + ns->THN_transportN + ns->MOW_transportN+  + ns->HRV_transportN;

		
	/* sum of current storage */
	ns->store = ns->leafn      + ns->leafn_storage      + ns->leafn_transfer +
