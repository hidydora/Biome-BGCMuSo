	         ...
	         metvar_struct* metv, epvar_struct* epv)
{
	...
	double lwRADnet, swRADnet, RADnet, RADnet_per_plaisun, RADnet_per_plaishade, RADcs, rad;
	...
	{
		printf("\n");
		printf("ERROR: Negative leaf carbon pool\n");
		printf("leafc = %.7e\n",cs->leafc);
		ok=0;
	} */
	...
	/* 2.2. net short-wave radiation: swRADnet. Dimensions: [rad]=MJ/m2/d-1; [swavgfd]=W/m2*/
	rad = metv->swavgfd * W_to_MJperDAY; 
	swRADnet = (1 - albedo_sw) * rad;
	...
	{
		printf("\n");
		printf("ERROR: actual vapor pressure is negative in radtrans.c (%lf)\n");
		ok=0;
	}
	
	/* lwRADnet */
	lwRADnet = STEFAN_BOLTZMANN * f_cd * (0.34 - 0.14 * sqrt(e_act)) * ((pow(Tmax_K,4) + pow(Tmin_K,4))/2);
	...
	metv->swRADnet = swRADnet / W_to_MJperDAY;
	metv->lwRADnet = lwRADnet / W_to_MJperDAY;
	metv->RADnet = RADnet / W_to_MJperDAY;
	metv->RADnet_per_plaisun = RADnet_per_plaisun / W_to_MJperDAY;
	metv->RADnet_per_plaishade = RADnet_per_plaishade / W_to_MJperDAY;
