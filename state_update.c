	                      ...
	                      cflux_struct* cf, nflux_struct* nf, cstate_struct* cs, nstate_struct* ns, int alloc, int evergreen)
{
	...
	if (alloc)
	{
		...
		if (!evergreen)
		{
			...
			if (cs->fruitc < CRIT_PREC || ns->fruitn  < CRIT_PREC) 
			{
				cs->fruitc = 0.0;
				ns->fruitn = 0.0;
			}
			...
			if (nsc_nw == 0 || (diff_total_nw - cf->NSC_nw_to_maintresp) > CRIT_PREC)
			{
				...
				if (cf->fruit_mr > 0)
				{
					if (cs->fruitc > CRIT_PREC)
					{
						...
						if (cf->fruitc_to_maintresp > cs->fruitc)
						{
							cf->fruitc_to_maintresp = cs->fruitc;
							excess += diff * (cf->fruit_mr / mresp_nw) - cf->fruitc_to_maintresp;
						}
