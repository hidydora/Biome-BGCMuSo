...

{
	...
	else
	{
		...
		{
			if (ok && transfer_fromGDD(ctrl, epc, cs, ns, phen, metv, epv, cf, nf))
			{
...
{
	double tsoilIMP;
	//double STRESSwater, STRESSthermal, STRESSnitrogen;
	...
	/* transfer period */
	if (n_actphen == epc->n_germ_phenophase)
	{
		/* determining the first and last day of transfer period */
		if (epc->n_germ_phenophase > 1)
		{
			phen->GDD_emergSTART = phen->GDD_crit[epc->n_germ_phenophase-2];
			phen->GDD_emergEND   = phen->GDD_crit[epc->n_germ_phenophase-2] + epc->phenophase_length[n_actphen-1];
		}
		else
		{
			phen->GDD_emergSTART = metv->GDD_wMOD;
		...
		if (metv->GDDpre == 0)
			...
			epv->transfer_ratio    = (metv->GDD_wMOD-metv->GDDpre)/(phen->GDD_emergEND-metv->GDDpre);
