...
int harvesting_init(file init, const control_struct* ctrl, harvesting_struct* HRV)
{
	...
	********************************************************************/
	...
	if (!errflag && scan_value(init, &HRV->HRV_num, 'i'))
	{
	...
	if (!errflag && HRV->HRV_num)
	{
		...
		if (!errflag && scan_value(init, HRV_filename, 's'))
		{
		...
		if (!errflag && scan_value(HRV_file, header, 's'))
		{
		...
		while (!errflag && !(mgmread = scan_array (HRV_file, &p1, 'i', 0, 0)))
		{
			n_HRVparam = 6;
