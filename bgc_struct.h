...
typedef struct
{
	...
	double RADnet_per_plaisun;               /* (W/m2) daylight average net radiation flux */
	double RADnet_per_plaishade;             /* (W/m2) daylight average net radiation flux */
...
typedef struct
{
	...
	double pondwater_src;			 /* SUM of water plus from goundwater */
...
typedef struct
{
	...
	double** seed_specweight_array;				/* (kgC/n) ARRAY of specific weight of seed */
...
typedef struct
{
	...
	double** Ccontent_array;					/* (kgC/kg fertilizer) ARRAY of carbon content of fertilizer */
    ...
    double** DC_NO3_array;				        /* (prop) ARRAY of dissolving coefficient of NO3 from fertilization */
	double** DC_NH4_array;				        /* (prop) ARRAY of dissolving coefficient of NH4 from fertilization */
	double** DC_C_array;				        /* (prop) ARRAY of dissolving coefficient of C from fertilization */
	double** EFfert_N2O;						/* (kgN2O-N:kgN) ARRAY of fertilization emission factor for direct N2O emissions */
	double FRZdepth_act;					    /* (m) actual  depth of fertilization */
	double DC_NO3_act;                          /* (prop) dissolving coefficient of NO3 from fertilization  */
	double DC_NH4_act;                          /* (prop) dissolving coefficient of NH4 from fertilization  */
	double DC_C_act;                            /* (prop) dissolving coefficient of C from fertilization  */
	double NO3_act;	                            /* (kgN/m2) nitrate amount in fertilizer pool */			
 	double NH4_act;	                            /* (kgN/m2) ammonium  amount in fertilizer pool*/			
	double CARBON_act;                          /* (kgC/m2) carbon amount in fertilizer pool  */	
	double flab_act;                            /* (%) labile fraction of fertilizer */
	double fucel_act;                           /* (%) unshielded cellulose fraction of fertilizer */
	double fscel_act;                           /* (%) shielded cellulose fraction of fertilizer */
	double flig_act;                            /* (%) lignin fraction of fertilizer */
	double EFf_N2O_act;                         /* (kgN2O-N:kgN) fertilization emission factor for direct N2O emissions from synthetic fertililers */
...
typedef struct
{
	...
	double cum_Closs_THN;				/* (kgC/m2)  cumulative SUM of thinning carbon loss  */
