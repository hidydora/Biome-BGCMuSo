...
int bgc(bgcin_struct* bgcin, bgcout_struct* bgcout)
{
	...
	if (ctrl.onscreen)
	{
		...
		fprintf(bgcout->control_file.ptr, "yday VWC1 LAI canopyw GPP TR EVAP\n");
	...
	for (simyr=0 ; ok && simyr<ctrl.simyears ; simyr++)
	{
		...
		for (yday=0 ; ok && yday<NDAY_OF_YEAR ; yday++)
		{
		   ...
		   /* FERTILIZING -  Hidy 2008 */
			...
			{
				fprintf(bgcout->control_file.ptr, "%i %f %f %f %f %f %f\n", 
				yday, 
				epv.vwc[0], epv.proj_lai, ws.canopyw, 
				summary.daily_gpp*1000, summary.daily_tr*1000, cf.m_leafc_to_SNSC); 
