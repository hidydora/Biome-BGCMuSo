...
#define N_SOILLAYERS 10		/* Hidy 2013 - number of type of soil layers in multilayer soil module */
...
typedef struct
{
    ...
    double vwc_avg;									/* (m3/m3) average volumetric water content in active layers - Hidy 2010 */
...
typedef struct
{
	double sand[N_SOILLAYERS];							/* (%)   Hidy 2016 - sand content of soil in the given soil layer */	
	...
	double soillayer_midpoint[N_SOILLAYERS];					/*  (m) Hidy 2016 - array contains the depths of the middle layers (positive values)*/
    double soil_b[N_SOILLAYERS];								/* (DIM) Hidy 2016 -Clapp-Hornberger "b" parameter */
	double BD[N_SOILLAYERS];									/* (g/cm3) Hidy 2016 -bulk density */
	double RCN;								                 	/* (m) Hidy 2016 - runoff curve number */
	double pondmax;								                 	/* (m) Hidy 2016 - maximum of pond water */
    double vwc_sat[N_SOILLAYERS];								/* (DIM) Hidy 2016 -volumetric water content at saturation */
    double vwc_fc[N_SOILLAYERS];								/* (DIM) Hidy 2016 -VWC at field capacity ( = -0.033 MPa) */
	double vwc_wp[N_SOILLAYERS];								/* (DIM) Hidy 2016 -VWC at wilting point ( = pF 4.2) */
	double vwc_hw[N_SOILLAYERS];								/* (DIM) Hidy 2016 -VWC at hygroscopic water point  ( = pF 6.2) */
    double psi_sat[N_SOILLAYERS];								/* (MPa) Hidy 2016 -soil matric potential at saturation */
	double psi_fc[N_SOILLAYERS];								/* (MPa) Hidy 2016 -soil matric potential at field capacity */
	double psi_wp[N_SOILLAYERS];								/* (MPa) Hidy 2016 -soil matric potential at wilting point */
