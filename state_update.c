	...
	if (mresp_nw)
	{
		...
		if (diff_total_nw > CRIT_PREC)
		{
			...
			if (nsc_nw == 0 || (diff_total_nw - cf->NSC_nw_to_maintresp) > CRIT_PREC)
			{
				...
				printf("\n");
				printf("WARNING: limited maintanance respiration of non-woody biomass (nsc_maintresp.c)\n");
			}
		}
	}
		...
		if (diff_total_w > CRIT_PREC)
		{
			...
			if (nsc_w == 0 || (diff_total_w - cf->NSC_w_to_maintresp) > CRIT_PREC)
			{
				...
				if (cf->livestem_mr > 0)
				{
					...
					{
						cf->livestem_mr       = 0;
						printf("ERROR: zero livestem maintanance respiration(nsc_maintresp.c)\n");
						ok = 0;
					}
				}
				...
				if (cf->livecroot_mr > 0)
				{
					...
					{
						cf->livecroot_mr       = 0;
						printf("ERROR: zero livecroot maintanance respiration(nsc_maintresp.c)\n");
						ok = 0;
					}
				}
				...
				printf("\n");
				printf("WARNING: limited maintanance respiration of woody biomass (nsc_maintresp.c)\n");
			}
		}
