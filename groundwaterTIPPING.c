	            ...
	            wstate_struct* ws, wflux_struct* wf, groundwater_struct* gws)
{
	...
	{
		sprop->GWeff[layer]=0;
		sprop->CapillFringe_act[layer] = 0;
	}
	...
	{	
		...
		if (sprop->GWlayer != DATA_GAP) sprop->CapillFringe_act[layer] = sprop->CapillFringe[layer];
		...
		layer = N_SOILLAYERS-1;
		while (GWdistM <= sprop->CapillFringe_act[layer] && layer >= 0)
		{
			...
			if (GWdistM <= sprop->CapillFringe_act[layer])
			{
				/* if groundwater table is in actual layer (above lower boundary): GWlayer = actual layer, lower layers are charged */
					...
					(layer < N_SOILLAYERS-1 && (sprop->VWCfc[layer+1]-epv->VWC[layer+1]) < critVWCdiff && sprop->VWCfc[layer+1] > sprop->VWCfc_base[layer+1]))
				{
					...
					if (GWdistM > 0)
						sprop->GWeff[layer] = 1-GWdistM/sprop->CapillFringe_act[layer];
					else
						sprop->GWeff[layer] = 1;
